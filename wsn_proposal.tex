...
\item Shooting and producing the videos: The videos will be shooted and produced according to the script.
...
\caption{It is necessary to shoot videos with step-by-step instructions to build the pilots.}
\label{fig:video}
\end{marginfigure}
...
\item introduction to arduino. Arduino IDE. Input/output. Blinking LED project.
\item Introduction to XBee. Basic configuration of AT mode. ZigBee chat project.
\item Basic interaction. Make a measurement and react. Wireless Sunset Sensor project.
\item Open data. The importance of sharing the data. Open data platforms. Taking measures with a sensor and uploading them to the Internet.
\end{enumerate}

Motivating videos:
\begin{itemize}
\item Bottom-up, Sensors, Smart Cities:
Michel Bauwens, Tiberius Brastaviceanu, Tomas Diez
...
\caption{Jaume Barcelo currently teaches a course on Wireless Sensor Networks at Universitat Pompeu Fabra}
\label{fig:jaume}
\end{marginfigure}
